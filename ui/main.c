...
void UI_DisplayMain(void)
{
    ...
    {
        unsigned int line0 = 0, line1 = 4;
        unsigned int line = isMainOnly() ? 0 : (vfo_num == 0 ? line0 : line1);
        bool isMainVFO = (vfo_num == gEeprom.RX_VFO);
...
#ifdef ENABLE_FEAT_F4HWN
        if (isMainOnly() && activeTxVFO != vfo_num)
            continue;
#endif
        ...
        if (IS_MR_CHANNEL(gEeprom.ScreenChannel[vfo_num]))
        {
            ...
            const bool inputting = gInputBoxIndex != 0 && gEeprom.RX_VFO == vfo_num;
            sprintf(String, inputting ? "M%.3s" : "M%u", inputting ? INPUTBOX_GetAscii() : gEeprom.ScreenChannel[vfo_num] + 1);
        ...
        else if (IS_NOAA_CHANNEL(gEeprom.ScreenChannel[vfo_num]))
        {
            const unsigned int x = 7;
            if (gInputBoxIndex == 0 || gEeprom.RX_VFO != vfo_num)
        ...
        else if (gInputBoxIndex > 0 && IS_FREQ_CHANNEL(gEeprom.ScreenChannel[vfo_num]) && gEeprom.RX_VFO == vfo_num)
